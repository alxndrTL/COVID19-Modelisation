class GridUpdater
{
  ...
  int aliveNeighbors(Cell[][] grid, int row, int col) //change variables ? x and y look like coordinate, but here these are indices
    //assume x, y are superior to 0, and inferior to grid.length-1 // TODO : gerer le cas ou x,y = 0 ou x,y = grid.length-1
  {
    int numAliveNeighbors = 0; // -1 to not count the cell itself as a neighbor
